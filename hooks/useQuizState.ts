...
import type { Question, SavedAnswer, ChoiceQuestion, MultiChoiceQuestion, FillQuestion } from "@/lib/types";
...
export function useQuizState(initialQuestions: Question[]): QuizHookReturn {
  ...
  const pickAnswer = useCallback((qi: number, oi: number) => {
    setDoneFlags((prev) => {
      ...
      if (q.type === "填空题" || q.type === "多选题") return prev;
